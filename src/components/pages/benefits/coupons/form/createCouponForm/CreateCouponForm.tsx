...
export default function CreateCouponForm() {
	...
	const {
		...
		watch,
		isValid,
	} = useFormHandler<CreateCouponFormValues>(createCouponSchema, defaultCreateCouponValues, 'all');
					...
					<div>
						{DISCOUNT_UNIT_TYPE_LIST.map(unit => (
							<button
								...
								onClick={(e) => {
									e.preventDefault();
									setValue('discountType', unit.value as keyof typeof DISCOUNT_UNIT_TYPE);
								}}
								className={discountUnitButton({ active: String(unit.value === watch('discountType')) })}
							>
								{unit.label}
							</button>
						))}
			...
			render: (field) => (
				<div className={styles.benefitInputBox}>
					...
					<LabeledCheckbox
						...
						onToggle={() => watch('amount') !== UNLIMITED_VALUE ? setValue('amount', UNLIMITED_VALUE) : setValue('amount', 0)}
					/>
				</div>
			),
		},
