...
import { NoticeListData } from "@/types/community";
import { useGetNoticeList } from "@/api/community/queries/useGetNoticeList";
import { Page } from "@/types/common";
import { useDeleteNotice } from "@/api/community/mutations/useDeleteNotice";
...
export default function NoticeList() {
	...
	const { data } = useGetNoticeList(page);
	...
	return (
		<CommunityList
			title='공지사항'
			data={data?.list as NoticeListData[]}
			page={page}
			onChangePage={onChangePage}
			pageData={data?.page as Page}
			columns={[
				{ key: 'title', header: '제목' },
				{ key: 'createdDate', header: '작성일', render: (row: NoticeListData) => format(new Date(row.createdDate), 'yyyy-MM-dd') },
				{ key: 'status', header: '노출 여부', render: (row: NoticeListData) => STATUS[row.status] },
			]}
