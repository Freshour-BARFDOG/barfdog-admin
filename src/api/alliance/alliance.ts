...
const mergeAllianceAndEventInfos = (
  baseList: AllianceManagementData[],
  extendedList: ExtendedAllianceData[]
) => {
  const map = new Map<string, ExtendedAllianceData>(
    extendedList?.map((e) => [e.allianceCode, e])
  );
  return (
    baseList?.map((item) => {
...
export const allianceManagementApi = {
  getAllianceManagement: async (
    ...
    try {
      const [{ data: baseData }, { data: eventData }] = await Promise.all([
        instance.get(
          `/api/admin/alliance/management?page=${page}&size=${PAGE_SIZE.COMMON}&${query}`
        ),
        instance.get(`/api/admin/alliance/event`),
      ]);

      const baseList =
        baseData?._embedded?.allianceCouponManagementResponseList;
  ...
  getAllianceDetail: async (
    allianceId: number,
    instance: AxiosInstance = axiosInstance
  ): Promise<AllianceDetailResponse> => {
    try {
      const { data } = await instance.get(
        `/api/admin/alliance/${allianceId}/detail`
      );
      return {
  ...
  deleteAllianceIdList: async (allianceIdList: number[]) => {
    try {
      const { data } = await axiosInstance.post("/api/admin/alliance/delete", {
        allianceIdList,
      });
      return data;
    } catch (error) {
...
export const allianceEventApi = {
  createAllianceEvent: async (body: CreateAllianceEvent) => {
    ...
    try {
      const { data } = await axiosInstance.post(
        `/api/admin/alliance/allianceEvent/create?${query}`
      );
      return data;
    } catch (error) {
      return handleApiError(error);
  ...
  deleteAllianceEventIdList: async (allianceEventIdList: number[]) => {
    try {
      const { data } = await axiosInstance.post(
        "/api/admin/alliance/allianceEvent/delete",
        {
  ...
  getAllianceEventList: async (
    instance: AxiosInstance = axiosInstance
  ): Promise<AllianceCouponSelectOption[]> => {
    try {
      const { data } = await instance.get(`/api/admin/alliance/event`);
      return (
        data?._embedded?.allianceEventResponseList.map(
          (item: ExtendedAllianceData) => ({
            label: item.allianceName,
            value: item.allianceId,
            eventInfoList:
              item?.eventInfos.map((event) => ({
...
export const allianceListApi = {
  getAllianceMemberList: async (
    ...
    try {
      const { data } = await instance.get(
        `/api/admin/members?page=${page}&size=${PAGE_SIZE.COMMON}&alliance=cb&${query}`
      );
      return {
        page: data.page,
  ...
  getAllianceSalesList: async (
    ...
    try {
      const { data } = await instance.get(
        `/api/alliance/payment?apiKey=${PAYMENT_API_KEY}&${query}`
      );
      return data;
    } catch (error) {
      return handleApiError(error);
...
export const allianceCouponApi = {
  getAllianceCouponList: async (
    ...
    try {
      const { data } = await instance.get(
        `/api/admin/alliance/coupon/${
          status === "ACTIVE" ? "created" : "deleted"
        }/history?page=${page}&size=${PAGE_SIZE.COMMON}&${query}`
      );
      return {
        page: data.page,
        couponList:
          data?._embedded?.queryAdminAllianceCouponEventResponseList || [],
      };
    } catch (error) {
      return handleApiError(error);
    }
  ...
  getAllianceCouponDetail: async (
    couponBundle: string,
    instance: AxiosInstance = axiosInstance
  ): Promise<AllianceCouponDetailResponse> => {
    try {
      const { data } = await instance.get(
        `/api/admin/alliance/coupon/created/detail?allianceCouponBundle=${couponBundle}`
      );
      return {
  ...
  deleteAllianceCouponList: async (couponBundleList: string[]) => {
    try {
      const { data } = await axiosInstance.post(
        "/api/admin/alliance/coupon/delete",
        {
  ...
  createAllianceCoupon: async (body: AllianceCouponFormValues) => {
    try {
      const { data } = await axiosInstance.post(
        "/api/admin/coupons/alliance/create",
        body,
        {
          timeout: 50000,
        }
  ...
  excelDownloadAllianceCoupon: async (body: ExcelDownloadAllianceCoupon) => {
    try {
      ...
      const { data } = await axiosInstance.get(
        `/api/admin/alliance/coupon/excel-download?${query}`,
        {
          responseType: "blob",
        }
      );
  ...
  excelDownloadCreateAllianceCoupon: async (
    body: ExcelDownloadCreateAllianceCoupon
  ) => {
    ...
    try {
      const { data } = await axiosInstance.post(
        `/api/admin/coupons/excel-download?${query}`,
        undefined,
        {
          responseType: "blob",
          timeout: 50000,
...
export const { getAllianceMemberList, getAllianceSalesList } = allianceListApi;
